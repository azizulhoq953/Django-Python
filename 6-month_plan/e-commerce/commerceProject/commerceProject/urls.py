"""
URL configuration for commerceProject project.

The `urlpatterns` list routes URLs to views. For more information please see:
    https://docs.djangoproject.com/en/4.2/topics/http/urls/
Examples:
Function views
    1. Add an import:  from my_app import views
    2. Add a URL to urlpatterns:  path('', views.home, name='home')
Class-based views
    1. Add an import:  from other_app.views import Home
    2. Add a URL to urlpatterns:  path('', Home.as_view(), name='home')
Including another URLconf
    1. Import the include() function: from django.urls import include, path
    2. Add a URL to urlpatterns:  path('blog/', include('blog.urls'))
"""
from django.contrib import admin
from django.urls import path
from mainApp import views 
from django.conf.urls.static import static
from django.conf import settings

admin.site.site_header = "Portfolio Administration"
admin.site.site_title = "Welcome To Portfolio Dashboard"
admin.site.index_title = "Welcome To This Portfolio"
admin.site.site_url ="/azizulhoq"

urlpatterns = [
    path('admin/', admin.site.urls),
    path( '',views. MainPage, name='shop'),
    path( 'products-details/',views. productDetail, name='details'),
    path( 'cart/',views. cartDetail, name='cart'),
    path( 'checkout/',views. checkOut, name='check'),
    path( 'pricing/',views. priceDetails, name='price'),
    path( 'login/',views. loginDetails, name='login'),
    path( 'register/',views. signup, name='signup'),
    path( 'profile/',views. profileDetails, name='profile'),



] + static(settings.MEDIA_URL, document_root=settings.MEDIA_ROOT)
